...
uses
  ...
  cxGridInplaceEditForm, Controller.AbrangenciaExpressas, Vcl.ExtCtrls, cxDropDownEdit, cxLookupEdit,
  cxDBLookupEdit, cxDBLookupComboBox, cxImageComboBox, Thread.ImportaAbrangencia;
...
uses Common.Utils, Common.ENum, Data.Module;
...
begin
  ...
  dm.mtbClientesEmpresa.Active := False;
end;
...
begin
  try
    ...
    if dm.mtbRoteirosExpressas.IsEmpty then Exit;

    if dm.SaveDialog.Execute() then
    begin
      if FileExists(dm.SaveDialog.FileName) then
      begin
        sMensagem := 'Arquivo ' + dm.SaveDialog.FileName + ' já existe! Sobrepor ?';
        if Application.MessageBox(PChar(sMensagem), 'Sobrepor', MB_YESNO + MB_ICONQUESTION) = IDNO then Exit
      end;

      fnUtil.ExportarDados(gridAbrangencia,dm.SaveDialog.FileName);
...
var
  FAbrangencia : TAbrangenciaExpressasController;
  aParam : Array of variant;
begin
  try
    FAbrangencia := TAbrangenciaExpressasController.Create;
    memTableAbrangencia.Active := False;
    sSQLOld := '';
    SetLength(aParam,2);
...
begin
  dm.PopulaClientesEmpresa;
  dm.mtbClientesEmpresa.Insert;
  dm.mtbClientesEmpresacod_cliente.AsInteger := 0;
  dm.mtbClientesEmpresanom_cliente.AsString := 'TODOS';
  dm.mtbClientesEmpresadom_check.AsInteger := 0;
  dm.mtbClientesEmpresa.Post;
  dm.mtbClientesEmpresa.First;
end;
